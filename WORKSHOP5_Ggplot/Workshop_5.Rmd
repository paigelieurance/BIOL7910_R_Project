...
par(las=1)
barplot(dat$total_bill,ylim=c(0,20),
        names.arg=dat$time,xlab='Time of day',
        col="#AFC0CB",ylab='Total bill',
        border='black')
box()
```
...
```{r, echo=FALSE,fig.asp=0.6,fig.height=2}
ggplot(data=dat, aes(x=time, y=total_bill, fill=time)) +
  geom_bar(colour="black", fill="#AFC0CB", 
           width=.8, stat="identity") +
  xlab("Time of day") + ylab("Total bill") +
...
# MISC GRAPHING - Drew's script - fix with headings and markdown formatting
...
# Paired histogram in base R
pdf('paired_histogram.pdf',width=6,height=4) #making a fixed pdf of graph

hist(data$value[data$type=='variable 1'], breaks=30, xlim=c(-3,8), col=rgb(1,0,0,0.5), xlab="value", 
     ylab="count", main="" )

# Second with add=TRUE to plot on top
hist(data$value[data$type=='variable 2'], breaks=30, xlim=c(-3,8), col=rgb(0,0,1,0.5), add=TRUE)

# Add legend
legend("topright", legend=c("var 1","var 2"), 
       col=c(rgb(1,0,0,0.5), rgb(0,0,1,0.5)), 
       pt.cex=2, pch=15,bty='n')

dev.off() #turns off pdf function


# Histogram using ggplot2
# Source: https://www.r-graph-gallery.com/histogram_several_group.html
data %>%
...
# type colours() in console to list colour options to sub into code
...
# base R
d <- density(data$price[data$price<300])
...
mtext(text='A boxplot with jitter',line=1) # adds title to figure

# enhanced boxplot using ggplot2
...
# Violin plot in base R
vioplot(value~name,data,xlab='',las=1)
mtext(text='A Violin wrapping a boxplot',line=1)

# Violin plot in ggplot2
# source: https://www.data-to-viz.com/graph/violin.html
data %>%
...
# base r
# source: https://www.r-graph-gallery.com/45-confidence-interval-around-polynomial-curve-fitting.html
plot(x,y,col=rgb(0.4,0.4,0.8,0.6), pch=16 , cex=1.3 , xlab="" , ylab="") 
model <- lm(y ~ x + I(x^2) + I(x^3))
myPredict <- predict(model, interval="prediction" )
...
# ggplot2
# source: https://www.r-graph-gallery.com/50-51-52-scatter-plot-with-ggplot2.html
data <- data.frame(
  x = x,
  y = y
)
...
# base R
plot(value~date,tail(data,n=10),type='l',las=3)
points(value~date,tail(data,n=10),pch=16)

# ggplot2
# source: https://www.r-graph-gallery.com/connected_scatterplot_ggplot2.html
data %>%
  tail(10) %>%
